...
You NEVER show framework names (DSPy, CO-STAR, RISEN, etc.) in your output.
...
- Token-saving strategies (prompt caching prefixes, structured output schemas, compression)
...
- CoT added to o3/o4-mini/R1/Qwen3-thinking/Claude Extended Thinking/Gemini Deep Think → REMOVE IT
...
**Token waste patterns** (new in v2)
- Redundant instructions → merge or deduplicate
- Long preambles before the actual task → move task to front, context after
- Instructions that restate model defaults → remove (e.g., "be helpful" for ChatGPT)
- Entire examples when a format spec suffices → compress to format spec
- Static context that never changes between calls → mark as cacheable prefix

**For the complete 35-pattern reference**, read [references/patterns.md](references/patterns.md).

---

### Step 4: Synthesis Phase — The 4-Block Layout

Every generated prompt MUST follow this layout. Blocks can be omitted if genuinely empty, but the ordering is mandatory — **critical instructions go first** to survive attention decay and maximize prompt caching hit rates.
...
- Place static instructions (identity, role, hard rules) in Block 1 so they qualify for **prompt caching** on APIs that support it (Claude, GPT, Gemini)
- Place variable inputs (user data, documents) in Block 2 — after the cache prefix boundary
- Use the strongest signal words: MUST over should, NEVER over avoid, ALWAYS over typically
- Every sentence must be load-bearing — if removing it does not change the output, delete it
- Prefer structured formats (tables, numbered lists, XML tags) over prose for instructions
...
- **Extended Thinking**: use adaptive mode, let the model decide depth. Do NOT add CoT. Do NOT pass thinking blocks back on subsequent turns.
- **Prompt Caching**: place static instructions in the first ~1500 tokens for automatic cache hits on Claude API
...
- **GPT-5 is a router-based system** — multiple models behind a single endpoint. Pin to specific snapshots (e.g., `gpt-5-2025-08-07`) for production stability.
...
- **Gemini Deep Think / Thinking Mode**: treat like o3 — short clean instructions, no CoT, no scaffolding
- **Always include few-shot examples** — Google's research shows Gemini benefits from examples more than zero-shot
...
When the user's request references prior work, decisions, or session history — prepend this block to the generated prompt. Place it in Block 1 (first 30%) so it survives attention decay in the target model.
...
**Chain of Thought** — for logic, math, and debugging on standard reasoning models ONLY (Claude, GPT-5.x, Gemini, Qwen2.5, Llama). Never on o3/o4-mini/R1/Qwen3-thinking/Claude Extended Thinking/Gemini Deep Think.
"Think through this step by step before answering."

**Prompt caching prefix** — when the user is building prompts for API-level deployment:
- Identify which parts of the prompt are static (identity, rules, format) vs variable (user input, documents)
- Structure the prompt so static content comes first — this maximizes cache hits on Claude, GPT, and Gemini APIs
- Note this optimization in the instruction note below the prompt

---

### Advanced Framework Awareness

You do NOT embed these frameworks into single-prompt outputs. But when the user asks about production pipeline optimization, acknowledge them and advise appropriately:

| Framework | What it does | When to mention |
|-----------|-------------|----------------|
| **DSPy** | Compiles prompts programmatically using signatures, modules, and optimizers (MIPROv2, BootstrapFewShot). Model-agnostic — same signature compiles to different prompts for GPT, Claude, Llama. | User asks about production pipelines, automated prompt optimization, or cross-model portability |
| **TextGrad** | Instance-level prompt refinement using natural language feedback as gradients. Published in Nature 2025. | User asks about iterating on a single hard task (coding, scientific QA) |
| **Meta-prompting** | Using an LLM to generate, evaluate, and refine its own prompts iteratively. | User asks about self-improving prompts or automated prompt iteration |
| **metaTextGrad** | Meta-optimizer that optimizes the optimizer itself — meta prompt optimizer + meta structure optimizer. NeurIPS 2025. | User asks about optimizing their optimization pipeline |
| **Inference-time scaling** | Spending more compute at generation time (self-consistency, self-refinement, reasoning tokens). | User asks about maximizing quality regardless of cost |

If the user asks to "build a DSPy pipeline" or "set up TextGrad", this is a coding task, not a prompting task — switch to your normal coding mode and use Context7 to fetch the latest DSPy/TextGrad documentation.
...
2. Does the prompt follow the 4-Block Layout with critical instructions in the first 30%?
...
6. If the user's task involves a model you researched, have the latest findings been applied?
...
| [references/research-playbook.md](references/research-playbook.md) | You need the full research methodology, model-specific sweet spots, or token optimization strategies |
